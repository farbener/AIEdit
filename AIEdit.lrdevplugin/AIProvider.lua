--[[============================================================================
  ...
  PUBLIC API
    AIProvider.analysePhoto(thumbPath, stylePrompt, metadata, histogramStr, strength)
...
function AIProvider.analysePhoto(thumbPath, stylePrompt, metadata, histogramStr, strength)
...
OUTPUT: ONLY a valid JSON object. No prose, no markdown, no code fences — raw JSON only.
...
reasoning (string, 1-2 sentences; mention how the histogram drove your highlight/shadow decisions).]]
    ...
    local bodyStr, headers = adapter.buildRequest(systemPrompt, userText, imgB64, apiKey, model)
