...
An Adobe Lightroom **Classic** plugin that sends a photo to an AI model (Anthropic Claude, OpenAI ChatGPT, or a local Ollama model), gets back a full set of develop adjustments tailored to that image, and applies them -- with a visual preview, presets, batch editing, and one-click revert.

It is built for portraits and tuned to get the **subject** correctly exposed without wrecking the background, using skin-tone metering rather than naive frame-average brightness. Everything runs locally inside Lightroom; the only network call is the single request to the provider you choose.
...
- **Three AI providers, switchable in the dialog** -- Anthropic Claude (`claude-sonnet-4-6`), OpenAI ChatGPT (`gpt-4o`), or a local **Ollama** model (`qwen2.5vl:7b` by default, no API key required). Keys are stored separately per provider.
...
- An API key for at least one cloud provider, **or** a locally running Ollama instance:
  - **Anthropic** -- <https://console.anthropic.com> -> API Keys
  - **OpenAI** -- <https://platform.openai.com> -> API keys
  - **Ollama** -- <https://ollama.com> (free, runs locally, no key needed). Install Ollama, then run `ollama pull qwen2.5vl:7b`.
- **API usage costs money** (cloud providers). You pay per request with your own key. A single edit sends one downsized JPEG plus a short text prompt, so cost per photo is small, but it is not zero. Ollama is free.
...
4. Select a photo, run **Library -> Plug-in Extras -> AI Edit Selected Photo...**, choose your provider, and paste your API key (it's saved automatically). For Ollama, no key is needed -- just make sure Ollama is running.
...
For each photo the plugin renders two JPEG thumbnails (1024px for cloud providers, 400px for Ollama), decodes one in pure Lua to compute a histogram, and sends the image + histogram + metadata + your style note to the chosen provider. The model returns a JSON object of develop adjustments, which the plugin maps to Lightroom develop keys and applies in one write (after snapshotting the prior state for revert).
...
- **Models** are pinned per provider in `AIEdit.lrdevplugin/AIProvider.lua` (the `MODELS` table). Edit those strings to use a different/newer model. All must be vision-capable.
- **Adding another provider** (e.g. Gemini) is a single entry in the `ADAPTERS` table plus one `MODELS` line -- no other file changes.
- **Plugin identifier** is `io.github.farbener.aiedit` (`Info.lua`). If you fork and redistribute, use your own identifier. Choose it **before** distributing -- changing it later orphans users' saved keys and presets.
...
- Each edit uploads a downsized JPEG of the photo plus a short text prompt to the chosen provider's API. Ollama runs entirely locally -- no data leaves your machine. Review each cloud provider's data-use policy if that matters to you.
...
- The system prompt was tuned with Claude; **OpenAI or Ollama may need minor prompt re-tuning** to match exposure discipline exactly.
- The SDK can't add toolbar/filmstrip buttons -- commands live in the Library menu.
- **Ollama image quality:** Ollama receives a 400px thumbnail (vs 1024px for cloud providers) to fit within local model context windows. For a 7B model this is sufficient, but very fine detail may be missed.
...
- **"Model returned invalid JSON"** -- for Ollama, make sure Ollama is running (`ollama serve`) and the model is pulled (`ollama pull qwen2.5vl:7b`). Check `response length=` in the log; if it's under 200 bytes, Ollama returned an error rather than a completion.
...
Built on the Adobe Lightroom Classic SDK and the Anthropic, OpenAI, and Ollama APIs.
